...
var application = function (yidConfig) {
    ...
    var templateHelperUser = function () {
        Handlebars.registerHelper('printUserNoButtons', function () {
            var ret = "";
            var message = "";
            if (this.isOnline) {
                if (this.isBusy) {
        ...
        Handlebars.registerHelper('printUser', function () {
            var ret = "";
            ret += '<div class="userName pull-left">';
            ret += Handlebars.helpers.printUserNoButtons.apply(this);
            ret += '</div>';
    ...
    applicationBuildTemplates = function () {
        self.templates.pluginTemplate = Handlebars.compile(self.config.pluginTemplate);
        self.templates.configurationTemplate = Handlebars.compile(self.config.configurationTemplate);
        self.templates.errorTemplate = Handlebars.compile(self.config.errorTemplate);
        self.templates.infoTemplate = Handlebars.compile(self.config.infoTemplate);
        self.templates.inCallParticipantsTemplate = Handlebars.compile(self.config.inCallParticipantsTemplate);
        self.templates.inCallParticipantTemplate = Handlebars.compile(self.config.inCallParticipantTemplate);
        self.templates.inCallSharesTemplate = Handlebars.compile(self.config.inCallSharesTemplate);
        self.templates.inCallLocalSharesTemplate = Handlebars.compile(self.config.inCallLocalSharesTemplate);
        self.templates.preCallSearchProgressTemplate = Handlebars.compile(self.config.preCallSearchProgressTemplate);
        self.templates.preCallMyRoomTemplate = Handlebars.compile(self.config.preCallMyRoomTemplate);
        self.templates.preCallSearchTemplate = Handlebars.compile(self.config.preCallSearchTemplate);
        self.templates.preCallSearchUserTemplate = Handlebars.compile(self.config.preCallSearchUserTemplate);
        self.templates.preCallSearchJustUserTemplate = Handlebars.compile(self.config.preCallSearchJustUserTemplate);
        self.templates.pluginInstallInstructionsTemplate = Handlebars.compile(self.config.pluginInstallInstructionsTemplate);
        self.templates.pluginEnableInstructionsTemplate = Handlebars.compile(self.config.pluginEnableInstructionsTemplate);
        self.templates.inCallChatTabTemplate = Handlebars.compile(self.config.inCallChatTabTemplate);
        self.templates.inCallChatPaneTemplate = Handlebars.compile(self.config.inCallChatPaneTemplate);
        self.templates.inCallChatPaneMessageTemplate = Handlebars.compile(self.config.inCallChatPaneMessageTemplate);
        self.templates.inCallParticipantInfoTemplate = Handlebars.compile(self.config.inCallParticipantInfoTemplate);
        self.templates.preCallInvitingDialogTemplate = Handlebars.compile(self.config.preCallInvitingDialogTemplate);
                ...
                if (results.total > 0) {
                    ...
                    $.each(results.Entity, function (i, entity) {
                        ...
                        transformedResult.fav.push({
                            ...
                            isLocked: false, //entity.RoomMode.isLocked,
                            hasPin: false, //entity.RoomMode.hasPIN,
...
$(document).ready(function () {
    logger.enable(yidveoConfig.enableAppLogs); // Don't show logs if configuration prohibits
    logger.enableStackTrace(yidveoConfig.enableStackTrace);
    logger.setLevelsCategories(yidveoConfig.applicationLevelsCategories);

    logger.log('info', 'application', 'domReady::DOM is loaded');

    try {
        var app = application(yidveoConfig);
        app.main();
